...
const Footer = ({ darkMode }) => {
  ...
  return (
    <footer className={`relative overflow-hidden bg-gradient-to-b ${bgGradient} text-white `}>
      <div className={`absolute -top-20 -right-20 w-64 h-64 rounded-full ${bgCircle} blur-3xl`}></div>
      <div className={`absolute -bottom-40 -left-40 w-80 h-80 rounded-full ${bgCircle} blur-3xl`}></div>
      ...
      <div className="container mx-auto px-6 py-16 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div>
            <h3 className={`text-lg font-semibold mb-6 pb-2 border-b ${borderColor} flex items-center`}>
              <Circle className="w-3 h-3 rounded-full bg-gradient-to-r from-purple-400 to-pink-400 mr-2" />
              Quick Links
            </h3>
            <ul className="space-y-3">
              {['Home', 'About Us', 'Donate', 'Campaigns', 'Impact Stories', 'Contact'].map((link) => (
                <li key={link}>
                  <Link
                    href="#"
          ...
          <div>
            ...
            <ul className="space-y-4">
              {[
                { icon: MapPin, title: "Address", content: "NIT PATNA BIHTA CAMPUS" },
                ...
                <li key={index} className="flex items-start">
                  ...
                  <div>
                    <h4 className={`font-medium text-purple-100 dark:text-purple-200`}>{title}</h4>
